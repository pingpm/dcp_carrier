...
const _sfc_main = {
  ...
  computed: {
    ...
    searchResults() {
      ...
      const matchedCities = this.allCities.filter((city) => this.regionMatches(city, keyword) || this.regionMatches(this.provinceMap[String(city.parentId)], keyword)).map((city) => this.decorateItem(city, "city"));
